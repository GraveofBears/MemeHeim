...
namespace CreatureManager
{
    ...
    public class Creature
    {
        ...
        public Creature(GameObject creature)
        {
            ...
            // --- Taming defaults ---
            if (creature.TryGetComponent(out Tameable tameable))
            {
                CanBeTamed = true;
                FedDuration = tameable.m_fedDuration;
                TamingTime = tameable.m_tamingTime;
                SpawnsTamed = tameable.m_startsTamed;
            }
            else
            {
                CanBeTamed = false;
            ...
            // --- Faction default ---
            if (creature.TryGetComponent(out Character character))
            {
                CreatureFaction = character.m_faction;
            }

            // --- Food defaults ---
            FoodItems = string.Join(",",
                creature.GetComponent<MonsterAI>()?.m_consumeItems
                    .Where(i => i.m_itemData.m_dropPrefab)
                    .Select(i => i.m_itemData.m_dropPrefab.name)
        ...
        internal static void Patch_FejdStartup()
        {
            ...
            foreach (Creature creature in registeredCreatures)
            {
                ...
                configWithDesc(cfg.FedDuration, () => creature.FedDuration, updateAI, "Fed Duration", new ConfigDescription("How long the creature stays fed after eating, in seconds.", null, tameConfigVisibility));
                configWithDesc(cfg.TamingTime, () => creature.TamingTime, updateAI, "Time to Tame", new ConfigDescription("The time it takes to tame the creature.", null, tameConfigVisibility));
                configWithDesc(cfg.SpawnsTamed, () => creature.SpawnsTamed ? Toggle.On : Toggle.Off, updateAI, "Spawns Tamed", new ConfigDescription("If on, the creature spawns tamed.", null, tameConfigVisibility));
                ...
                config(cfg.CreatureFaction, () => creature.CreatureFaction, updateCharacterAttributes, "Creature Faction", "The creature faction the creature belongs to.");
            }
        ...
        {
            CreatureConfig cfg = creatureConfigs[this];
            character.m_faction = cfg.CreatureFaction.get();
        }
